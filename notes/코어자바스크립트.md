...
# 데이터 타입
-  기본형 타입은 값이 직접 메모리에 저장되지만, 참조형 타입은 값이 위치한 주소만 저장
- 따라서 참조 타입을 수정하면 주소는 그대로 두고 대상 값의 내용이 변경됨
## primitive type
- 기본형
- number
- string
...
- LexicalEnvironment
	...
	- outerEnvironmentReference: 외부 환경에 대한 참조
		...
		- scope chain: 변수의 유효 범위, 외부로는 들어갈 수 있는데 자기보다 안쪽으로 들어갈 수는 없다 쉽게 말하면 밑에 있는 실행컨텍스트들은 참조 가능하지만 위는 불가능
...
- 프로토타입 프로퍼티에 내부에 할당되지 않고 array 생성자 함수 객체에 직접 할당되어 있는 프로퍼티들을 스태틱 메서드, 스태틱프로퍼티라고 함
	...
	- 해당 클래스 소속의 인스턴스들의 개별적인 동작이 아닌 소속 여부 확인, 소속 부여 등의 공동체적인 판단을 필요로 하는 경우에 스태틱 매서드를 활용하곤 함
- 한편, 프로토타입 내부에 정의된 메서드들을 일컬어 포로토타입 메서드라고 하는데, 앞의 프로토타입을 생략하고 메서드라고 많이 부름
	- 프로토타입 프로퍼티는 인스턴스와 [ [ prototype ] ]으로 연결 되어있고 이것이 곧 인스턴스와 겹쳐서 동작하기 때문에 인스턴스에서 다이렉트로 프로토타입 메서드에 접근이 가능한 반면에 생성자 함수 내부에 있는 프로퍼티나 메소드는 다이렉트로 접근할 방법이 없음
- 클래스에는 인스턴스에서는 직접 접근할 수 없는 클래스 자체에서만 접근가능한 스태틱 멤버와 인스턴스에서 직접 활용할 수 있는 프로토타입 메서드가 있음

- Employee.prototype = new Person();
	- Employee.prototype과 Person의 인스턴스를 연결
- Employee.prototype.constructor = Employee;
	- Employee.prototype.constructor에 다시 Employee를 부여해서 원래있던 프로토타입 객체와 같은 기능을 수행하도록 함
- Employee.prototype.getPosition = function(){ return this.position; }


- function Bridge() {}
	- 아무런 프로퍼티도 생성하지 않는 비어있는 객체를 생성하는 생성자 함수
	- 둘 사이에 다리를 놓는다는 의미로 bridge
	- Person과 Employee 둘 사이
- Bridge.prototype = Person.prototype;
	- bridge의 프로토타입에 person의 프로토타입을 연결한 상태에서 인스턴스를 생성하면 그 인스턴스에는 아무런 프로퍼티가 없이 메서드만 상속받는 형태가 됨
- Employee.prototype = new.Bridge();
	- 그 상태에서 Employee의 프로토타입과 Bridge의 프로토타입 연결
	- 원래라면 person의 인스턴스를 넣던 대신에 빈 객체를 넣어주는 것
- Employee.prototype.constructor = Employee;
	- 마지막으로 본래의 프로토타입 기능을 되살리기 위해 constructor을 연결해주면 끝남


- var extendClass = (function(){ function Bridge(){}}
...
- 그러나 최근에는 class Person을 만들고 class Employee extends Person을 해주면 바로 상속관계 구현가능