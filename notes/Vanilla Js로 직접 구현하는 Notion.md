...
## 애플리케이션의 핵심 3계층
- 데이터 계층
	- 모든 애플리케이션 상태를 state 객체에 중앙 관리
	- load( )와 save( )함수로 브라우저 저장소와 동기화하여 앱 재시작 시 상태 복원을 보장함
- UI 렌더링 계층
	- 데이터 변화에 따라 renderTrees, renderPage 등으로 DOM을 재 렌더링하여 화면과 상태를 동기화함
	- 상태 -> 화면 의 단방향 데이터 흐름을 유지함
- 상호작용 계층
	- 이벤트 리스너와 라우팅으로 사용자 행동을 감지하고 상태 변경
	- 사이드 바 클릭 시 화면이 갱신되며, 에디터 입력 내용이 즉시 저장
	- 다양한 사용자 상호작용을 처리
